...
void get_rates(int thresh_min, int thresh_max, int thresh_step) {
	...
	while (pch != NULL) {
		if (count==3) sprintf(datetimelabel,"%s_",pch);
		if (count==4) sprintf(datetimelabel,"%sh",pch);
		if (count<5) {
	...
	for (threshold=loopmax; threshold>=loopmin; threshold-=thresh_step) {

		// set the tresholds on the crate and wait.
		printf("Setting a threshold of %i\n",threshold);
		if (doADC==1) {
			sprintf(SystemCallString,"echo faSetThresholds %i",threshold); // it appear that the scalers use 
